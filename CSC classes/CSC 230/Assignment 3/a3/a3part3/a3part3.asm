...
start:
	...
	check_timer3:
		...
		push r16
		push r17
		rcall lcd_gotoxy		;go to row 0 column 14 on LCD
		pop r17
		pop r16
		...
		ldi temp, 0x20			;case for blank pulse
		push temp
		rcall lcd_putchar
		pop temp
		ldi temp, 0x20
	...
	no_pulse:	
		ldi r16, 1				;go to second row column 11 on LCD
		ldi r17, 11
		push r16
		push r17
		rcall lcd_gotoxy
		...
		rcall to_decimal_text		;Use provided code to turn BUTTON_COUNT to decimal
		pop temp
		pop temp
		pop temp
		pop temp
		...
		ldi r16, 1			;go to second row column 0 on LCD
		ldi r17, 0
		push r16
		push r17
		rcall lcd_gotoxy
		...
		lds temp, BUTTON_CURRENT		;check to see if button is being pressed
		cpi temp, 0x00
		breq off
		ldi r20, 0x06			;if yes place 6 *'s on LCD
		ldi temp, '*'		
		loop2:
			 push temp
			 rcall lcd_putchar
...
off:	ldi r20, 0x06				;if no button is being pressed, place space on LCD
		ldi temp, ' '
		loop3:
			 push temp
			 rcall lcd_putchar
...
timer4:
	...
	lds temp, BUTTON_CURRENT		;check for case when BUTTON_CURRENT is 1 and BUTTON_PREVIOUS is 0
	cpi temp, 0x00
	breq end4
	
	lds temp, BUTTON_PREVIOUS
	cpi temp, 0x01
	breq end4
	
	case_01:		;increment BUTTON_COUNT
